...
import constants.DropdownConstatnts;

public final class PerformAction extends UIDriver {

	// private static final Logger LOG = Logger.getLogger(PerformAction.class);
	 ...
	 * @param text
	 */
	public static void fillTextField(String domKey, String domValue, String text) {
		String name = domKey.split("_")[2];
		// System.out.println("name :-" + name);
		WebElement locator = getLocator(domKey, domValue);
		...
		try {
			if (locator.isDisplayed()) {
				if (locator.isSelected()) {
					...
					// Select the checkbox
					locator.click();
					// LOG.info("Clicked on " + "'" + name + "'" + " checkbox");
